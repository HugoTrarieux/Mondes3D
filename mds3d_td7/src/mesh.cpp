...
void Mesh::draw(const Shader &shd)
{
    ...
    int texcoords_loc = shd.getAttribLocation("vtx_texcoords");
    if(texcoords_loc>=0)
    {
        glVertexAttribPointer(texcoords_loc, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)(2*sizeof(Vector3f)+sizeof(Vector4f)));
    ...
    if(texcoords_loc>=0)  glDisableVertexAttribArray(texcoords_loc);
