...
void Mesh::loadOBJ(const std::string& filename)
{
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        ...
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
            ...
            for (size_t v = 0; v < (unsigned long) fv; v++) {
                // access to vertex
...
bool Mesh::intersectFace(const Ray& ray, Hit& hit, int faceId) const
{
    ...
    Point3f q = T.cross(e1);
    Vector3f d = (ray.direction);
    Point3f p = d.cross(e2);

    float t = q.dot(e2) / p.dot(e1);
    float u = p.dot(T) / p.dot(e1);
    float v = q.dot(d) / p.dot(e1);

    if(t<0 || u<0 || v<0 || u>1 || v>1 || u+v<0 || u+v>1){
        return false;
    } else{
        hit.setT(t);
        Normal3f normal = a.normal*u + b.normal*v + c.normal *(1-u-v);
...
bool Mesh::intersect(const Ray& ray, Hit& hit) const
{
    ...
    for(unsigned long i=0; i<m_faces.size(); i++){
        intersectFace(ray, hitFace, i);
        if(hitFace.foundIntersection()){
            if(hitFace.t()<hit.t()){
                hit.setT(hitFace.t());
